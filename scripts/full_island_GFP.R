...
icam_cases <- st_read("./data/gfp_cases_shapefile.gpkg")
...
cases_with_all <- cases_with_all %>%
  left_join(dplyr::select(wealth_index_matched, fs_uid, wealth_index), by = c("clinic_ID" = "fs_uid"))
...
plot(events_model, select = 1, shade = TRUE, shade.col = "lightblue",
     ...
     main = "Effect of Chlorophyll-a on General Food Poisoning Events", xlim=c(0,30),ylim = c(-1,1.5))
...
plot(events_model, select = 3, shade = TRUE, shade.col = "lightblue",
     ...
     main = "Effect of Precipitation on General Food Poisoning events", xlim=c(0,30),ylim = c(-1.5,1.5))
