...
ICAM_cases <- file5 %>% dplyr::select(clinic_ID,cAnnee, cPeriode, fname, V394:V407) 
...
ICAM_cases <- ICAM_cases %>%
  mutate(icam_total = rowSums(dplyr::select(., V394:V407), na.rm = TRUE))
...
st_write(export_data,"./data/gfp_cases_shapefile.gpkg", append = FALSE)
