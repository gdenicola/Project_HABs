...
diagnosis_dictionary <- read_csv("./data/diagnosis_dictionary_translated_MC_20250116.csv")
...
#variables V410-V423 contain ICAM cases (V424-V425 ICAM "referrals")
file5_new <- read_feather("./data/file5_new.feather")
...
ICAM_cases_new <- file5_new %>% dplyr::select(clinic_ID,cAnnee, cPeriode, fname, V410, V411, V412, V413, V414, V415, V416, V417, V418, V419, V420, V421, V422, V423) 

#ICAM data not available before 2015, so remove previous years
ICAM_cases <- ICAM_cases %>%
  filter(cAnnee >= 2016 & cAnnee <= 2022)
#ICAM data not available before 2015, so remove previous years
ICAM_cases_new <- ICAM_cases_new %>%
  filter(cAnnee >= 2016 & cAnnee <= 2022)

#ICAM data not available before 2015, so remove previous years
ICAM_cases_truncated <- ICAM_cases_new %>%
  filter(cAnnee >= 2016 & cAnnee <= 2019)
...
#combine ICAM cases with healthshed spatial data
combined_data_new <- ICAM_cases_new %>%
  inner_join(healthsheds_2022, by = c("clinic_ID" = "fs_uid"))

n_distinct(combined_data$clinic_ID)
n_distinct(combined_data_new$clinic_ID)


#keep only public clinics/hospitals
combined_data <- combined_data %>% filter(fs_type %in% c("CSB1", "CSB2")) 
combined_data_new <- combined_data_new %>% filter(fs_type %in% c("CSB1", "CSB2")) 
...
combined_data_new <- mutate(combined_data_new, icam_event = ifelse(icam_total > 0, 1, 0))
sum(combined_data$icam_event)
...
combined_data_new <- mutate(combined_data_new, large_icam_event = ifelse(icam_total > 3, 1, 0))

sum(combined_data$large_icam_event)
sum(combined_data_new$large_icam_event)

combined_data_new_nogeom <- combined_data_new %>% select(-geometry)
combined_data_nogeom <- combined_data %>% select(-geometry)

# Sort both datasets by clinic_ID, cAnnee (year), and cPeriode (month)
combined_data_new_nogeom <- combined_data_new_nogeom %>%
  arrange(clinic_ID, cAnnee, cPeriode)

combined_data_nogeom <- combined_data_nogeom %>%
  arrange(clinic_ID, cAnnee, cPeriode)

# Now, run the comparison again
all.equal(combined_data_nogeom, combined_data_new_nogeom)
...
# Summarize data by year, month and clinic_ID, summing up the icam_total, icam_event, and large_icam_event
summary_by_month_clinic_new <- combined_data_new %>%
  group_by(cAnnee, cPeriode) %>%
  summarise(icam_total_sum = sum(icam_total, na.rm = TRUE),icam_event_sum = sum(icam_event, na.rm = TRUE),icam_large_event_sum = sum(large_icam_event, na.rm = TRUE)) 
...
ggplot(combined_data_allyears) +
  geom_sf(aes(fill = ifelse(icam_event_sum == 0, NA, icam_event_sum)), color = "grey100") +
  scale_fill_viridis_c(option = "plasma", direction = -1, na.value = "white") +
  theme_void() +
